...
#  parent_adjlist: list of (list of (list of simplices for which this simplex is a face) for each simplex) for each dimension
...
class Filtration:
    ...
    def initialize_simplex_graph(self, verbose=False):
        # 3-dimensional array of (dimension, simplex, parents)
        self.parent_adjlist = [[[] for i in range(len(self.simplex_indices[k]))] for k in range(self.max_dimension)]
        for k in range(self.max_dimension):  # from 0 to (max_dimension - 1)
            for i, sim in enumerate(self.all_simplices[k]):
                for p in self.ordered_points:  # for each point
                    ...
                    if p in sim.points:
                        continue
                    parent = Simplex(points=(sim.points | {p}))
                    if verbose:
                        print("sim = " + str(sim))
                        print("parent = " + str(parent))
                    self.parent_adjlist[k][i].append(self.simplex_indices[k+1][parent])
        if verbose:
            print("parent_adjlist:")
            print(self.parent_adjlist)
            ...
            while new_simplices_queue:
                ...
                for new_face in new_simplices_queue:
                    ...
                    if k == self.max_dimension:
                        continue
                    # Otherwise, check each parent of this simplex
                    face_index = self.simplex_indices[k][new_face]
                    for sim_index in self.parent_adjlist[k][face_index]:
